...
| `web_interface.py` | `webInterface` | Full-screen `QWebEngineView` for smart-TV web browsing (see `WEB_INTEGRATION.md`). Injects custom spatial-nav JS (`window.__broNav`) for D-pad focus movement, reports the focused element back as a `FocusedElement` (duck-typed `.rect` contract that `inputInterface.setSelectedButton()` accepts). Handles incognito profiles and hands off text entry to the GUI keyboard overlay (dropping to GUI mode while it is open). |
